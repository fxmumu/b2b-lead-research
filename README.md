...
兼容 Codex、Claude Code、Cursor、ZCode 与 WorkBuddy（同一 SKILL.md 格式），其他宿主通过能力清单扩展。
...
脚本行为：
- 在 `~/.claude/skills/`、`~/.codex/skills/`、`~/.cursor/skills/`、`~/.zcode/skills/`、`~/.workbuddy-ai/skills/` 下各建一个 symlink 指向仓库本体
...
scripts/install.sh --hosts claude-code # 指定宿主（已知：claude-code, codex, cursor, zcode, workbuddy）
```
...
# workbuddy: ~/.workbuddy-ai/skills/b2b-lead-research
```
