...
支持断点续跑：中间产物增量落盘（`lead-research/` 目录），任务可跨会话恢复。
...
```bash
cp config/leads.yaml.example config/leads.yaml
```

`config/leads.yaml` 已被 `.gitignore` 排除——它包含你的公司信息、ICP 和市场策略，保留在本地，不上传。
